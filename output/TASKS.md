...
## NOW
- [x] **[CRITICAL] UNDER-only strategy locked in (2026-04-15)** — Fallback policy now has `allow_over: False` / `min_edge_over: 9.9` for pts, pra, pr, pa, stl, blk. Dynamic policy has matching hard-block. Only TRB/AST/TOV OVER remain eligible (evidence ambiguous). STL/BLK structural UNDER picks re-enabled (`allow_structural_micro_props: True`). See DEC-034.
- [x] **[CRITICAL] Absence threshold — recency gate added (2026-04-15)** — `detect_recent_absences()` in `usage_injury_model.py` now requires that a player with low `gp` ALSO missed the most recent team game date before flagging. Players who appeared in the last game (Curry-style load management) are no longer marked absent. See DEC-032.
- [x] **[CRITICAL] Step 8 duplicate-player fix + 17.7% missing-feature fix (2026-04-15)** — (1) All layer source DataFrames (`bl`, `adj`, `prof`, `luck`) now `drop_duplicates` before merging. (2) All name-based merges (Layers 3, 4, 5) now use `player_norm` (NFKD ASCII-normalized) as the join key. `proj["player_norm"]` is added once before the Layer 5 merge and shared across all three name-based layers. Diacritic players (Jokić, Dončić, etc.) now match correctly instead of filling with zeros. See DEC-033.
